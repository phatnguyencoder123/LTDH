...
class MainApp(tk.Tk):
    def __init__(self):
        ...
        self.input_app.grid(row=0, column=0, padx=50, pady=10)

        # Khởi tạo OutputApp, truyền dữ liệu từ InputApp vào
        self.output_app = OutputApp(container, self)
        self.output_app.grid(row=0, column=2, padx=50, pady=10)

        self.mohrcircle = MohrsCircleApp(container, self)  # <-- Kiểm tra kỹ tên biến!
        self.mohrcircle.grid(row=0, column=1, padx=50, pady=10)

        self.showobject = ShowObjectApp(container, self)
        self.showobject.grid(row=0, column=3, padx=50, pady=10)
